  ...
  padding-right: 5%;">
 	<div class="container-fluid" style="background-color: white;box-shadow: 5px 10px 8px #888888;">
          ...
          <?php
                  ...
                  $q1="SELECT * from studentmst  where Studentid!=$sid";
                  $res1=mysqli_query($cnn,$q1);
                  while($ans1=mysqli_fetch_array($res1))
                  {
                      $rid=$ans1['Studentid'];
                      $img=$ans1['Profile'];
                      $name=$ans1['Middlename']; 
              ?>
            <div class="col-sm-3">
              
                <?php 
                      // $q2="SELECT * from teacherfriendmst where Sender=$sid and Receiverstudent=$rid union SELECT * from studentfriendmst where Receiverteacher=$sid and Sender=$rid";
                      $q2="SELECT * from studentfriendmst where Sender=$sid and Receiverstudent=$rid union SELECT * from studentfriendmst where Sender=$rid and Receiverstudent=$sid";
                                ...
                                if($ans2['Status']==1)
                                {
                                  ?>
                                   <div id="div1">
                                    ...
                                    <a href=""   class="btn  btn-primary w-100"  role="button"  name="cancel" id="<?php echo $rid; ?>"><i class="fa fa-smile-o" aria-hidden="true"></i>&nbsp;Your Friends</a>
                                  </div>  
                                <?php
                                }
                                else
        ...
        <?php 
            if(isset($_REQUEST['frd']))
            {
                ...
                $rid1=$_REQUEST['rid'];
                $time=time();
                $q3="INSERT into studentfriendmst(Sender,Receiverstudent,Time) values($sid1,$rid1,'$time')";
                //echo $q3;
            ...
            if(isset($_REQUEST['can']))
            {
                $sid2=$_REQUEST['sid'];
                $rid2=$_REQUEST['rid'];
                $q4="DELETE from studentfriendmst where Sender=$sid2 and Receiverstudent=$rid2";
